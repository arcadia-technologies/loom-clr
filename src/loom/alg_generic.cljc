...
(def bits-per-long (long #?(:default 64 :cljs 32)))

(defn bm-longs
  "Returns the number of longs required to store bits count bits in a bitmap."
  [bits]
  (long #?(:clj (Math/ceil (/ bits bits-per-long))
           :cljr (Math/Ceiling (/ bits bits-per-long)))))
...
(defn- bm-copy ^longs [bm size]
  #?(:clj (java.util.Arrays/copyOf ^longs bm ^System.Int64 size)
     :cljr (let [^longs arr2 (make-array System.Int64 ^System.Int64 size)]
             (Array/Copy ^longs bm, arr2, ^System.Int64 size))
...
(defn bm-set
  ...
  (let [size (max (count bitmap) (bm-longs (inc idx)))
        ...
        new-value #?(:clj (bit-or value ^Long mask)
                     :cljr (bit-or value ^System.Int64 mask))]
